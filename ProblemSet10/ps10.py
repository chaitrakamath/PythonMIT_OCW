...
class Hand(object):
    ...
    def update(self, word):
        ...
        newhand = {} #create a copy so that orginal hand dictionary should not have any side effects
    ...
    def containsLetters(self, letters):
        ...
        """
        # TODO
        # print 'self.hand:', self.handDict
        freq = getFrequencyDict(letters)
        for l in letters:
            if freq[l] > self.handDict.get(l, 0):
                return False
        return True
    ...
    def isEmpty(self):
        ...
        """
        # TODO
        if len(self.handDict) == 0: return True #if hand is empty
        if all(value <= 0 for value in self.handDict.values()): return True #all characters have 0 values in dictionary
    ...
    def __eq__(self, other):
        ...
        """
        # TODO
        #check if lengths of both dictionaries are same. If not, return False
        isEqual =  len(self.handDict) == len(other.handDict)
        if not isEqual:
            return isEqual

        #is all letters from other.handDict exist in self.handDict, the dictionaries are equal
        isEqual = all(letter in other.handDict for letter in self.handDict) 
        if not isEqual: 
            return isEqual

        #if all keys are equal, make sure that all values are same
        isEqual = all(other.handDict[letter] == self.handDict[letter] for letter in self.handDict)
        return isEqual
...
class ComputerPlayer(Player):
    ...
    def pickBestWord(self, wordlist):
        ...
        # TODO
        wl = wordlist.getList()
        # print 'subset of wordlist:', wl[10:20]
        score_dict = {}
        hand = self.getHand()
        # print 'hand:', hand
        for word in wl:
            if hand.containsLetters(word): #word can be formed with given hand
                score_dict[word] = getWordScore(word)
