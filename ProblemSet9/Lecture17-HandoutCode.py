...
class Drunk(object):
	...
	def move(self, field, time = 1):
		if field.getDrunk != self: raise ValueError('Drunk.move called with drunk not in field')
		for i in range(time):
			pt = CompassPt(random.choice(CompassPt.possibles))
			field.move(pt, 1)

def performTrial(time, fieldObj):
	start = fieldObj.getLoc()
	distances = [0.0]
	for t in range(1, time + 1):
		f.getDrunk().move(fieldObj)
		newLoc = fieldObj.getLoc()
