...
def is_valid_word(word, hand, points_dict):
    ...
    word: string
    hand: dictionary (string -> int)
    points_dict: dictionary of word:score as key value pairs
    """
    freq = get_frequency_dict(word)
    for letter in word:
...
def pick_best_word(hand, points_dict):
    ...
    for word in points_dict:
        if is_valid_word(word, hand, points_dict):
            newScore = get_word_score(word, sum(hand.values()))
            possibleWords[word] = newScore
    bestWord = max(possibleWords, key = possibleWords.get)
    return bestWord
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    timeAllowed = float(raw_input('Enter time limit, in seconds, for players: '))
    while sum(hand.values()) > 0:
        ...
        if userWord == '.':
            ...
            if not isValid:
                ...
                if total_time < 1: #if user enters word quickly, do not divide points by total_time
                ...
                if total_time < timeAllowed:
                    print 'You have', str(timeAllowed - total_time), 'seconds remaining'
                    timeAllowed -= total_time
                    total += points #update totals only if word was provided within set time limit
                else:
                    print 'Total time exceeds', timeAllowed, 'seconds. You scored', int(total), 'points'
                    break;
